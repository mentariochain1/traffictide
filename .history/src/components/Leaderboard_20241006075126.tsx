...
const Leaderboard: React.FC = () => {
  ...
  const userPosition = useMemo(() => {
    return userEntry ? sortedLeaderboard.findIndex(entry => entry.id === userEntry.id) + 1 : 0;
  }, [sortedLeaderboard, userEntry]);

  const userName = useMemo(() => {
    return user ? `${user.first_name}${user.last_name ? ' ' + user.last_name : ''}` : userEntry?.name || 'Unknown User';
  }, [user, userEntry]);
