...
const EditProfile: React.FC<EditProfileProps> = ({ onClose, initialData }) => {
  ...
  const shouldUseDarkText = useCallback((color: string): boolean => {
  ...
  const textColor = useMemo(() => {
    return shouldUseDarkText(formData.avatarBorderColor) ? '#000000' : '#FFFFFF';
  }, [formData.avatarBorderColor, shouldUseDarkText]);
  ...
  useEffect(() => {
    ...
    if (webApp) {
      ...
      try {
        const safeColor = formData.avatarBorderColor && formData.avatarBorderColor.length >= 7
          ? formData.avatarBorderColor
          : '#4F46E5';
        webApp.MainButton.color = safeColor;
        webApp.MainButton.textColor = textColor;
      } catch (error) {
        console.error('Error setting MainButton color:', error);
