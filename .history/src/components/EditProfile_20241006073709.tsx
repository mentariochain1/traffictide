...
const EditProfile: React.FC<EditProfileProps> = ({ onClose, initialData }) => {
  ...
  const shouldUseDarkText = useCallback((color: string | null | undefined): boolean => {
    if (!color) return true; // По умолчанию используем темный текст
    const match = color.match(/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i);
    if (!match) return true; // Если цвет некорректный, используем темный текст
  ...
  useEffect(() => {
    ...
    if (webApp) {
      ...
      webApp.MainButton.color = color || '#FFFFFF';
