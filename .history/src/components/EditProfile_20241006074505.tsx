...
const EditProfile: React.FC<EditProfileProps> = ({ onClose, initialData }) => {
  ...
  const textColor = useMemo(() => {
    return shouldUseDarkText(formData.avatarBorderColor || '#FFFFFF') ? '#000000' : '#FFFFFF';
  ...
  useEffect(() => {
    ...
    if (webApp) {
      ...
      // Используем безопасный цвет по умолчанию
      const safeColor = formData.avatarBorderColor || '#4F46E5';
